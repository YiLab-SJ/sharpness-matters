"""
Demographic Analysis for VinBig and SIIM-ACR Datasets

This script analyzes the demographic distribution (age and sex) of medical imaging datasets
using the VinDataset or ChestXrayDataset (SIIM-ACR) classes. It generates comprehensive statistics and visualizations.

Usage:
    # Analyze VinDR dataset (default)
    python sharpness_matters/ptx/datasets/extract_demographics.py
    
    # Analyze VinDR dataset explicitly
    python sharpness_matters/ptx/datasets/extract_demographics.py --dataset vindr
    
    # Analyze SIIM-ACR dataset (test split by default)
    python sharpness_matters/ptx/datasets/extract_demographics.py --dataset siim_acr
    
    # Analyze SIIM-ACR train split
    python sharpness_matters/ptx/datasets/extract_demographics.py --dataset siim_acr --split train
    
    # Analyze both train and test splits of SIIM-ACR
    python sharpness_matters/ptx/datasets/extract_demographics.py --dataset siim_acr --split both
    
    # Specify output directory
    python sharpness_matters/ptx/datasets/extract_demographics.py --dataset siim_acr --split both --output-dir /path/to/output
"""
...
SIIM_ROOT_DIR = cfg.ptx.siim_acr.root_dir
VINDR_ROOT_DIR = cfg.ptx.vindr.root_dir


def analyze_demographics(
    dataset: Any, dataset_name: str = "Unknown", output_dir: str = f"{HOME}/output"
) -> Tuple[Dict[str, Any], pd.DataFrame]:
    """
    Analyze the demographic distribution of a medical imaging dataset.

    Args:
        dataset (Any): Dataset instance (VinDataset or ChestXrayDataset)
        dataset_name (str): Name of the dataset for labeling outputs
        output_dir (str): Directory to save results and plots

    Returns:
        Tuple[Dict[str, Any], pd.DataFrame]: Dictionary containing all demographic statistics and DataFrame with demographics data
    """
    ...
    output_csv = os.path.join(output_dir, f"{dataset_name.lower()}_demographics.csv")
    df.to_csv(output_csv, index=False)
    logger.info(f"Demographics saved to {output_csv}")

    # Save summary statistics
    output_json = os.path.join(
        output_dir, f"{dataset_name.lower()}_demographic_stats.json"
    )
...
def analyze_age_distribution(
    ...
    Returns:
        Dict[str, Any]: Dictionary containing age statistics
    """
    age_stats = {}
    ...
    # Debug: Check age data types and values
    logger.info(f"Age data types: {age_data['age'].dtype}")
    logger.info(f"Age data sample: {age_data['age'].head(10).tolist()}")
    ...
    if len(age_data) > 0:
        ...
        age_group_counts_series = age_data["age_group"].value_counts()
        age_group_counts = age_group_counts_series.to_dict()
        ...
        plt.title("Age Distribution by Pneumothorax Label")
        plt.ylabel("Age (years)")
        plt.grid(True, alpha=0.3)

        # Age groups bar plot
        plt.subplot(1, 3, 3)
        age_group_counts_series.plot(kind="bar")
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, f"{dataset_name.lower()}_age_distribution.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def analyze_sex_distribution(
    ...
    Returns:
        Dict[str, Any]: Dictionary containing sex statistics
    """
    sex_stats = {}
    ...
    if len(sex_data) > 0:
        ...
        plt.title("Sex Distribution by Pneumothorax Label")
        plt.xlabel("Sex")
        plt.ylabel("Count")
        ...
        plt.title("Sex Proportions by Pneumothorax Label")
        plt.xlabel("Sex")
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, f"{dataset_name.lower()}_sex_distribution.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def analyze_crosstabs(
    ...
    Returns:
        Dict[str, Any]: Dictionary containing cross-tabulation statistics
    """
    crosstab_stats = {}
    ...
    if len(complete_data) > 0:
        ...
        plt.title("Age Group × Pneumothorax Label")
        plt.ylabel("Age Group")
        ...
        plt.title("Sex × Pneumothorax Label")
        plt.ylabel("Sex")
        plt.xlabel("Label (0=Neg, 1=Pos)")
        ...
        plt.savefig(
            os.path.join(
                output_dir, f"{dataset_name.lower()}_demographic_crosstabs.png"
            ),
            dpi=300,
            bbox_inches="tight",
        )
...
def print_summary(stats: Dict[str, Any]) -> None:
    ...
    Args:
        stats (Dict[str, Any]): Dictionary containing demographic statistics
    """
    ...
    print(f"  Positive samples (pneumothorax): {stats['positive_samples']:,}")
...
def create_split_comparison(
    combined_df: pd.DataFrame, output_dir: str, dataset_name: str = "Dataset"
) -> None:
    """
    Create comparison visualizations between train and test splits.

    Args:
        combined_df (pd.DataFrame): Combined DataFrame with train/test splits
    ...
    plt.tight_layout()
    filename = f'{dataset_name.lower().replace("-", "_").replace(" ", "_")}_split_comparison.png'
...
def load_siim_dataset(split: str = "test") -> Any:
    """
    Load SIIM-ACR dataset with default parameters.

    Args:
        split (str): Data split to load ('train' or 'test')

    Returns:
        Any: ChestXrayDataset instance
    """
    # Default paths for SIIM-ACR dataset - you may need to adjust these
    dicom_dir = SIIM_ROOT_DIR
    label_dir = os.path.join(SIIM_ROOT_DIR, "train-rle.csv")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Analyze demographics of medical imaging datasets"
    )
    parser.add_argument(
        "--dataset",
        choices=["vindr", "siim_acr"],
        default="vindr",
        help="Dataset to analyze: vindr or siim_acr",
    )
    parser.add_argument(
        "--split",
        choices=["train", "test", "both"],
        default="test",
        help="Data split to analyze: train, test, or both (only for SIIM-ACR)",
    )
    parser.add_argument(
        "--output-dir",
    ...
    try:
        ...
        if args.dataset == "vindr":
            dataset = VinDataset(img_size=224)
            dataset_name = "VinDR"
            ...
            if args.split == "both":
                ...
                train_dataset = load_siim_dataset(split="train")
                train_dataset_name = "SIIM-ACR Train"
                ...
                test_dataset = load_siim_dataset(split="test")
                test_dataset_name = "SIIM-ACR Test"
                ...
                combined_csv = os.path.join(
                    args.output_dir, "siim_acr_combined_demographics.csv"
                )
                combined_df.to_csv(combined_csv, index=False)
                logger.info(f"Combined demographics saved to {combined_csv}")

                # Create split comparison visualization
                create_split_comparison(combined_df, args.output_dir, "SIIM-ACR")

                logger.info("Analysis completed for both splits!")
                return

            else:
                dataset = load_siim_dataset(split=args.split)
                dataset_name = f"SIIM-ACR {args.split.title()}"
        ...
        logger.info(
            f"  - {dataset_name.lower()}_demographics.csv: Detailed demographics data"
        )
        logger.info(
            f"  - {dataset_name.lower()}_demographic_stats.json: Summary statistics"
        )
        logger.info(
            f"  - {dataset_name.lower()}_age_distribution.png: Age analysis plots"
        )
        logger.info(
            f"  - {dataset_name.lower()}_sex_distribution.png: Sex analysis plots"
        )
        logger.info(
            f"  - {dataset_name.lower()}_demographic_crosstabs.png: Cross-tabulation heatmaps"
        )
