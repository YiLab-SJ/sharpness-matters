...
Usage:
    Run this module directly to execute the experiments:
        python3 sharpness_matters/pneumonia/evaluation/evaluate_saliency_ood.py
...
def run_experiment(
    ...
    """
    # For prediction we use batch_size=1 to preserve order.
    ...
    cache_dir = f"{HOME}/output/chunk_predictions"
    os.makedirs(cache_dir, exist_ok=True)
    # For each fold, load the checkpoint and run predictions in chunks.
    for fold in range(5):
        if device.type == "cpu":
        ...
        if "densenet" in model_name or "resnet" in model_name:
            ...
            raise Exception("Unknown model name. Choose one of densenet, resnet")
        predict_fold_chunked(
            pred_loader,
            model,
            target_layers,
        ...
        del model
        gc.collect()
    # Now compute ensemble scores by reading the chunked predictions.
    ...
    # Save the computed scores and a histogram.
    save_path = f"{HOME}/output/saliency_eval/{model_name}"
    os.makedirs(save_path, exist_ok=True)
    np.save(os.path.join(save_path, f"bboxoodiou_{img_size}_{threshold}.npy"), iou)
    np.save(
        os.path.join(save_path, f"bboxoodprecision_{img_size}_{threshold}.npy"),
        precision,
    )
...
if __name__ == "__main__":
    seed = 42
    batch_size = (
        1  # For prediction we use 1 to preserve order; chunking is done manually.
    )
        ...
        for model_name in model_names:
            ...
            for img_size in [64, 128, 224, 512, 768, 1024]:
                ...
                dataset = ChestXrayOOD(
                    train_names,
                    [
                        f"{DATA_DIR}/covid_study_level.csv",
                        f"{DATA_DIR}/covid_image_level.csv",
                    ],
                    img_size=img_size,
                )
                logger.info(
                ...
                out.append(res)
                df = pd.DataFrame(out)
                cols = ["img_size"] + [c for c in df.columns if c not in ["img_size"]]
                df = df[cols]
                df.to_csv(
                    f"{HOME}/output/saliency_bbox_{model_name}_ood_{threshold}.csv",
                    index=False,
                )
